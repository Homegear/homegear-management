...
void startUp()
{
	try
	{
    	...
    	else
    	{
    		...
    		GD::out.printInfo("Info: Homegear Management is (now) running as user with id " + std::to_string(getuid()) + " and group with id " + std::to_string(getgid()) + '.');
    	}
...
int main(int argc, char* argv[])
{
	try
    {
    	...
    	// {{{ Load settings
			...
			if((!GD::runAsUser.empty() && GD::runAsGroup.empty()) || (!GD::runAsGroup.empty() && GD::runAsUser.empty()))
			{
				GD::out.printCritical("Critical: You only provided a user OR a group for Homegear Management to run as. Please specify both.");
				exit(1);
			}
