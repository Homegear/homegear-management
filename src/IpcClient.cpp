...
Ipc::PVariable IpcClient::installNode(Ipc::PArray &parameters) {
  try {
    ...
    if (url.empty()) { //Node-RED node
      ...
      auto installCommand = (GD::bl->settings.nodeOptions().empty() ? "" : "NODE_OPTIONS=" + GD::bl->settings.nodeOptions() + " ") + homegearNodePath + " /usr/share/homegear/nodejs/lib/node_modules/npm/bin/npm-cli.js install --no-audit --no-update-notifier --no-fund --save --save-prefix=~ --production --color false " + module + " 2>/dev/null";
...
Ipc::PVariable IpcClient::uninstallNode(Ipc::PArray &parameters) {
  try {
    ...
    if (BaseLib::Io::directoryExists(nodeRedNodesPath + "node_modules/" + module)) {
      ...
      if (!BaseLib::Io::fileExists(homegearNodePath)) homegearNodePath = "/usr/bin/homegear-node";
      if (!BaseLib::Io::fileExists(homegearNodePath)) homegearNodePath = "homegear-node";
      auto uninstallCommand = (GD::bl->settings.nodeOptions().empty() ? "" : "NODE_OPTIONS=" + GD::bl->settings.nodeOptions() + " ") + homegearNodePath + " /usr/share/homegear/nodejs/lib/node_modules/npm/bin/npm-cli.js remove --no-audit --no-update-notifier --no-fund --save --color false " + module + " 2>/dev/null";
